...
module.exports = async options => {
    const debug2 = `ps aux | grep bpl`; // TODO remove
    const { stdout: debug2Out } = await exec(debug2);
    console.log(`[pool-clear] ${debug2} result : ${debug2Out}`);

    const commandReconnectNode = `docker network connect nodes $(docker ps --format "{{.Names}}" | grep node1_bpl) --alias node1 --ip $(cat dist/node1/ip.log)`;
    ...
    const commandLaunch = `docker ps --format "{{.Names}}" | grep node1_bpl | xargs -I {} sh -c 'docker exec -d {} bash bpl.sh'`;
